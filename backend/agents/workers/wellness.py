...
class WellnessAgent(BaseAgent):
    ...
    def run(self, state: dict) -> dict:
        ...
        message = "Here are some wellness recommendations tailored for you."

        return {
            "messages": [AIMessage(content=message)],
            "next": None,
            "worker_results": {
                "wellness_plan": "basic_recommendation"
            }
        }
...
def wellness_agent_node(state: dict):
    return wellness_agent.execute(state)